...
contract AIXTreasury is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event TotalDistribution(
        uint256 inputAixAmount,
        uint256 distributedAixAmount,
        uint256 swappedEthAmount,
        uint256 distributedEthAmount
    );
    ...
    function distribute(uint256 amount, uint256 minETHPerAIXPrice) external nonReentrant {
        ...
        if (remainingAIX > 0) {
            ...
            emit TotalDistribution(amount, amount * totalAixShares / DENOMINATOR, amounts[1], totalETH);
        } else {
            emit TotalDistribution(amount, amount * totalAixShares / DENOMINATOR, 0, 0);
        }
